...
import { getBrandLogo, sortBrands } from "@/lib/brandLogos";
              ...
              {brands.map((brand) => {
                ...
                return (
                  <Link
                    ...
                    className="carousel-item h-20 w-20 sm:h-28 sm:w-28 md:h-40 md:w-40 flex-shrink-0 transition-transform duration-300 hover:scale-105"
                    style={{
                      position: "relative",
      ...
      <section>
        <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-8 lg:gap-10 px-4 sm:px-6 md:px-10 lg:px-14 py-8 sm:py-10 md:py-12 lg:py-14 max-w-7xl mx-auto">
          {products.map((item, index) => {
            ...
            return (
              <FadeIn key={item.docId} delay={(index % 8) * 60}>
                <div className="border border-[#ccc] rounded-lg bg-[#222222] p-3 sm:p-4 flex flex-col transition-transform duration-300 hover:-translate-y-1">
                  <Link
